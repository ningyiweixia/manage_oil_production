...
│   │   ├── seed.py                  # 种子数据（17 菜单/44 权限/6 角色/93 字典/1 管理员）
...
│           ├── SystemAdminView.vue   # 系统管理（用户/角色/菜单/权限/数据字典/日志 Tab）
│           └── DictionaryManageView.vue # 数据字典管理（可嵌入系统管理）
│
├── deploy/                          # 部署与基础设施
...
└── tests/
    └── backend/
        ...
        └── test_a5_sync_service.py       # A5 同步统计与失败重试测试
```
...
GET    /api/v1/approval-logs           审批日志查询（默认 100 条，最多 500 条）
```

删除用户时，系统会先清理用户角色关系，并将关联审批日志、项目创建人字段置空，避免历史审计数据因外键约束被级联删除。
...
字典表维护字典类型、措施类型、项目状态、运行状态、审批动作、业务类型、系统角色、系统菜单、外部系统等枚举。接口层通过 `ensure_dictionary_values` 按 `item_value` 强制校验，种子数据会把历史中文措施值迁移为稳定编码。
...
DELETE /api/v1/workover-project-pools/{id}            删除归档（is_deleted=true，列表隐藏）
...
项目列表返回 `rejected_from_status` 字段，标明驳回项目在被驳回前所处的审批节点，前端据此区分「地质驳回」与「工艺驳回」。项目审批通过后进入“已入库”，系统自动创建对应修井运行表；派工后项目状态推进为 `DISPATCHED`。
...
审批通过的项目会自动生成一条 `WAITING_DISPATCH` 修井运行表。运行表列表和待派工列表加载时会兜底同步已入库项目，避免漏建运行表。
...
A5 回调或日报同步会写入运行表的 `a5_status`、`a5_remark`、`last_a5_sync_at`，并按 A5 状态同步施工状态、进度和承包商可用状态。
...
- 工单回调和日报同步统一写入修井运行表，保留 A5 状态、备注、最近同步时间和原始回写快照
- A5 状态会归一化为本系统运行表状态：下发/审核中 → `DISPATCHED`，施工中 → `WORKING`，办结/完成 → `FINISHED`，驳回/退回 → `WAITING_DISPATCH`，关闭/取消 → `CANCELED`
- 异常情况和特殊工序按同步日期写入 Redis 缓存，统计接口支持 `start_date`、`end_date`、`category` 筛选
- 全量同步包含日报、异常、特殊工序三部分；任一部分失败会记录 `partial_failure` 并抛出业务异常，交给 Celery 重试链路处理
...
- 审批操作：通过（流转到下一节点，审批通过后进入已入库并自动建运行表）、驳回（退回修改）
- 重新提报：根据 `rejected_from_status` 智能路由到地质核实或工艺核实
- 措施 JSONB 表单：动态增删措施行，措施类型下拉从字典 API 加载，提交前校验措施类型必填且不重复
...
- 审批状态柱状图
- 措施类型饼图（按数据字典展示中文名称）
- 区块 × 状态热力图（覆盖草稿、待审、已入库、已派工、已驳回）
- 日提报趋势折线/柱状混合图
- 图表一键导出 PNG + 数据摘要文本导出

### A5 集成前端

- 同步状态卡片：最近同步时间、同步结果、当日同步次数、运行中状态
- 异常情况统计：展示 A5 异常总数和按异常类别聚合的列表
- 特殊工序统计：展示 A5 特殊工序总数和按工序类别聚合的列表
- SSO 跳转：输入井号生成 5 分钟有效跳转链接
...
- 用户管理：新增用户时校验复杂密码，角色选择收敛为单角色分配，超级管理员禁止删除
- 角色/菜单/权限管理：支持 RBAC 资源维护和角色授权
- 数据字典：在系统管理页内统一维护，支持类型选择、启停、编辑和删除
- 日志审计：操作日志和审批日志分离查询，便于追踪系统操作与业务流转
...
| `20260629_0005` | 2026-06-29 | 支持用户物理删除，审批日志操作人和项目创建人外键改为 `ON DELETE SET NULL` |
| `150795c9dad6` | 2026-06-30 | 移除项目池 `VOIDED` 状态，历史作废记录归并为逻辑删除数据 |
| `20260630_0006` | 2026-06-30 | 修井运行表新增 A5 状态、备注和最近同步时间字段 |
...
- ✅ 系统管理前端：用户、角色、菜单、权限、数据字典、操作日志入口逐页验证通过
- ✅ 用户物理删除流程：超级管理员保护，审批日志操作人和项目创建人置空
- ✅ `python -m pytest`
