...
$env:CORS_ALLOW_ORIGINS = 'http://127.0.0.1:5174'
$env:REDIS_URL = ''
Start-Transcript -Path '__BACKEND_LOG__' -Force
try { & '__PYTHON__' -m uvicorn main:app --host 127.0.0.1 --port 8001 }
finally { Stop-Transcript }
'@.Replace('__REPO_ROOT__', $repoRoot).Replace('__PYTHON__', $python).Replace('__BACKEND_LOG__', (Join-Path $logRoot 'backend.transcript.log'))
$frontendScript = @'
...
Start-Transcript -Path '__FRONTEND_LOG__' -Force
try { & '__NODE__' '__VITE__' --host 127.0.0.1 --port 5174 --strictPort }
finally { Stop-Transcript }
'@.Replace('__FRONTEND_ROOT__', (Join-Path $repoRoot 'frontend')).Replace('__NODE__', $node).Replace('__VITE__', $vite).Replace('__FRONTEND_LOG__', (Join-Path $logRoot 'frontend.transcript.log'))
...
cmd.exe /c ('start "cross-module-backend" /min powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand {0}' -f $backendEncoded)
cmd.exe /c ('start "cross-module-frontend" /min powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand {0}' -f $frontendEncoded)
```

Expected: 启动命令采用项目验证过的 `cmd.exe /c 'start "title" /min ...'` 形式，后端在 `http://127.0.0.1:8001/docs` 提供服务，前端在 `http://127.0.0.1:5174` 提供服务。子进程在编码的 PowerShell 中设置 SQLite/mock、CORS 和 Vite API/WS 环境变量；日志写入 `.local/logs/`。`--strictPort` 禁止 Vite 静默换用其他端口；命令不依赖任何未跟踪的 `.local/*.ps1` 文件。
...
$expectedListeners = @(
  [pscustomobject]@{ Name = 'backend'; Port = 8001; Executable = $python; CommandLinePattern = '*uvicorn*main:app*--port 8001*' },
  [pscustomobject]@{ Name = 'frontend'; Port = 5174; Executable = $node; CommandLinePattern = '*vite.js*--port 5174*--strictPort*' }
)
$ownedProcesses = foreach ($expected in $expectedListeners) {
  $listeners = @(Get-NetTCPConnection -State Listen -LocalPort $expected.Port -ErrorAction Stop)
  if ($listeners.Count -ne 1) { throw "Expected exactly one listener on port $($expected.Port)" }
  ...
  if ($process.Path -ne $expected.Executable -or $commandLine -notlike $expected.CommandLinePattern) {
    throw "Listener on port $($expected.Port) is not the expected $($expected.Name) process"
  }
  [pscustomobject]@{ Name = $expected.Name; Port = $expected.Port; Id = $process.Id; StartTime = $process.StartTime; Executable = $process.Path; CommandLine = $commandLine }
}
```

Expected: 两个 HTTP 请求均成功；之后仅从 8001/5174 的监听器登记 PID，并校验其可执行文件与命令行分别是预期的 Python/Uvicorn、Node/Vite。记录 PID、启动时间、端口、响应状态和日志路径到验收记录，继续保持受管进程运行以完成任务 2 和任务 3。
...
```powershell
Invoke-WebRequest http://127.0.0.1:8001/openapi.json -OutFile .local\openapi.json
```
...
try {
  ...
  foreach ($owned in $ownedProcesses) {
    ...
    $commandLine = if ($process) { (Get-CimInstance Win32_Process -Filter "ProcessId = $($owned.Id)" -ErrorAction SilentlyContinue).CommandLine }
    if (-not $process -or $process.StartTime -ne $owned.StartTime -or $process.Path -ne $owned.Executable -or $commandLine -ne $owned.CommandLine) {
