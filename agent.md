...
* **项目名称**：采油二厂井下作业管理系统
* **项目定位**：一个集多方协同、多级审批、A5 系统联动、数据分析于一体的综合性井下作业全流程线上管理平台
* **核心价值**：实现闭环的业务管理流程，打通 A5 系统联动链路，提升审批进度透明度，并将海量业务数据转化为直观图表赋能决策
* **当前阶段**：核心业务模块全部实现，系统处于功能完善和稳定性增强阶段（对应原路线图阶段五）

## 二、 核心业务流程

该系统包含三个关键业务阶段，开发时需严格遵循此业务逻辑：
...
* **数据处理与报表生成**：`Pandas` (数据清洗统计与 Excel 解析) + `openpyxl` (Excel 报表生成)
...
前端 (Vue 3 + Element Plus) → Nginx (DMZ 反代)
  → FastAPI (应用逻辑层) → PostgreSQL + Redis (数据层)
...
* **外部系统客户端**：A5 系统客户端（`a5_client.py`）通过配置化的 base_url 进行连接，支持环境变量注入。
...
当前版本不包含工程设计/FPM/MinIO 模块。A5 异常与特殊工序统计可通过 `openpyxl` 生成 Excel 报表，并由前端以文件下载方式交付。

## 十二、 数据分析与统计模块

### 后端分析引擎
* **服务**：`services/workover_analytics_service.py`
* **技术实现**：常规列表查询使用 SQLAlchemy 条件下推数据库；统计图表和 Excel 导入清洗可使用 Pandas 做聚合与二次处理
* **严禁**：为普通分页列表查询加载全量数据到内存
...
* **深度安全审计**：权限变更、审批流转、派工调度等高危操作，审计探针强制记录变更前后数据快照（Data Snapshot），存储于 `approval_log.before_snapshot` 和 `after_snapshot` JSONB 字段
...
### AI 编写测试规范
* 新功能必须包含单元测试
* 关键业务路径（审批流转、派工调度、A5 同步）需包含集成测试
* 外部依赖（A5）需进行 Mock
...
| 阶段三 | 7-10周 | 对接 A5 系统、异常/特殊工序统计与 Excel 报表导出 | ✅ 已完成 |
...
* **菜单项**：系统管理、审批工作台、数据分析、承包商调度、A5集成、数据字典等
* **权限项**：覆盖所有 CRUD 操作的接口级权限
